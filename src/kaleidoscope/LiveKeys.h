...
///
/// - `Key_Transparent` indicates that the given key is not active. Usually this
///   will correspond to the keyswitch being off (not pressed).
///
/// - `Key_NoKey` indicates that the given key has been masked. When a key
...
class LiveKeys {
 public:
  ...
  /// Deactivate an entry by setting its value to transparent.
  void clear(KeyAddr key_addr) {
    if (key_addr.isValid())
      key_map_[key_addr] = Key_Transparent;
  }

  /// Mask a key by setting its entry to `Key_NoKey`. The key will become
  ...
  /// Clear the entire array by setting all values to `Key_Transparent`.
  void clear() {
