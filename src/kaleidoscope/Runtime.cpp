...
Runtime_::loop(void) {
  ...
  // trigger a call to `handlePhysicalKeyEvent()`, which in turn will
...
// ----------------------------------------------------------------------------
void
Runtime_::handlePhysicalKeyEvent(KeyEvent event) {
  ...
  auto result = Hooks::onPhysicalKeyEvent(event);

  // If an event handler changed `event.key` to `Key_NoKey` in order to mask
  ...
  // `Key_NoKey`, but I think this makes more sense. Note: It is still important
  // to let events with `Key_NoKey` to fall through to `handleKeyEvent()`,
  // because some plugins might still care about the event regardless of its
  // `Key` value, and more importantly, that's where we clear masked keys that
  // have toggled off. Alternatively, we could call `live_keys.clear(addr)` for
  // toggle-off events here, and `mask(addr)` for toggle-on events, then return,
  // short-cutting the call to `handleKeyEvent()`. It should work, but some
  // plugins might be able to use that information.
...
Runtime_::handleKeyEvent(KeyEvent event) {
  ...
  // For events that didn't begin with `handlePhysicalKeyEvent()`, we need to
  // look up the `Key` value from the keymap (maybe overridden by `live_keys`).
  if (event.addr.isValid()) {
  ...
  if (result != EventHandlerResult::OK ||
      event.key == Key_NoKey || event.key == Key_Transparent)
    return;
...
Runtime_::prepareKeyboardReport(const KeyEvent &event) {
  ...
  for (KeyAddr key_addr : KeyAddr::all()) {
    ...
    // If the key is idle, it should be transparent; if it's masked, it will be
    // Key_NoKey. Either way, we can ignore it.
