...
class Runtime_ {
 public:
   ...
   * job is to call the `onPhysicalKeyEvent()` plugin handler functions, used by
   ...
   */
  void handlePhysicalKeyEvent(KeyEvent event);

  /** Handle a logical key event
   *
   * This method triggers the handling of a logical "key event". Ususally that
   * event is the result of a call to `handlePhysicalKeyEvent()`, but it can
   * also be called by plugins that need to generate extra events without a 1:1
